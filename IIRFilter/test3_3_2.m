...
[bb2,aa2]=cheby2(a2,As,b2,'high','s');
h2=freqs(bb2,aa2,wc);
...
[bb3,aa3]=ellip(a3,Rp,As,b3,'high','s');
h3=freqs(bb3,aa3,wc);
...
[B1,A1]=cheby1(N1,Rp,WOW1,'high','s');
[D1,C1]=bilinear(B1,A1,Fd);
hh1=freqz(D1,C1,w);
[gd1,wd1]=grpdelay(D1,C1);
figure(1);
subplot(2,2,1);plot(w,20*log10(abs(hh1)),'b');hold on;
 title('高通双线性幅频特性(取对数)');ylabel('幅度 dB');xlabel('w/rad');
% set(gca,'XTickMode','manual','XTick',[ 0.458*pi 0.6*pi]);
% set(gca,'YTickMode','manual','YTick',[ -15  0]);
% grid;
...
[B2,A2]=cheby2(N2,As,WOW2,'high','s');
...
[B3,A3]=ellip(N3,Rp,As,WOW3,'high','s');
...
title('高通双线性幅频特性');ylabel('幅度 ');xlabel('w/rad');
...
title('高通双线性相位特性');ylabel('相位 ');xlabel('w/rad');
...
subplot(2,2,4);plot(wd1,(gd1),'b');hold on;
 title('高通双线性群延时');ylabel('时间 ');xlabel('w/rad');
