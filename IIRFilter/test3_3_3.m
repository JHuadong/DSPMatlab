...
title('带通幅频特性(取对数)');ylabel('幅度 dB');xlabel('w/rad');
% set(gca,'XTickMode','manual','XTick',[ 0.5*pi 0.6*pi]);
% set(gca,'YTickMode','manual','YTick',[-50 -15 -1 0]);
% grid;
...
title('带通双线性幅频特性');ylabel('幅度 ');xlabel('w/rad');
...
subplot(2,2,3);plot(w,(angle(hh1)),'b');hold on;
 title('带通双线性相位特性');ylabel('相位 ');xlabel('w/rad');
...
title('带通双线性群延时');ylabel('时间 ');xlabel('w/rad');
